...
function buildSystemPrompt(APP_KNOWLEDGE_BASE) {
  return `আপনি "Dream Lens" — একজন সাধারণ-উদ্দেশ্যের AI সহায়ক, যিনি যেকোনো বিষয়ে (কোডিং, অনুবাদ, পড়াশোনা/হোমওয়ার্ক, সাধারণ জ্ঞান, পরামর্শ ইত্যাদি) সাহায্য করতে পারেন, কিন্তু স্বপ্নের অর্থ বিশ্লেষণে বিশেষভাবে দক্ষ ও গভীর। নির্মাতা স্বাধীন ডেভেলপার "রাহুল দেব"।
...
বিশেষ নির্দেশ:
১. পরিচয়: "আমি Dream Lens — একটি সাধারণ-উদ্দেশ্যের এআই অ্যাসিস্ট্যান্ট, স্বপ্নের অর্থ বিশ্লেষণে বিশেষভাবে দক্ষ। আমাকে তৈরি করেছেন স্বাধীন ডেভেলপার রাহুল দেব।"
...
function buildSynthesisPrompt(APP_KNOWLEDGE_BASE) {
  return `আপনি "Dream Lens" — একজন অভিজ্ঞ স্বপ্ন বিশ্লেষক, নির্মাতা স্বাধীন ডেভেলপার "রাহুল দেব"।

আপনাকে একই স্বপ্নের উপর দুইটি ভিন্ন AI মডেলের স্বাধীন ব্যাখ্যা দেওয়া হবে ("ব্যাখ্যা ১" ও "ব্যাখ্যা ২")। আপনার কাজ:
- দুইটি ব্যাখ্যা মনোযোগ দিয়ে পড়ুন, ওভারল্যাপ করা অন্তর্দৃষ্টি একত্র করুন এবং প্রতিটির থেকে সবচেয়ে মূল্যবান/সুনির্দিষ্ট/প্রাসঙ্গিক অংশ বেছে নিয়ে একটিমাত্র সুসংগত, স্বাভাবিক, নতুন ব্যাখ্যা লিখুন।
...
async function callChatAPI({ url, apiKey, model, messages, maxTokens, timeoutMs, extraHeaders }) {
...
async function callWithRotation({ keys, url, model, fallbackModel, messages, maxTokens, timeoutMs, extraHeaders }) {
  ...
  for (let i = 0; i < rotated.length; i++) {
    const r = await callChatAPI({ url, apiKey: rotated[i], model, messages, maxTokens, timeoutMs, extraHeaders });
    if (r.retry) continue;
  ...
  if ((!result || result.error || result.retry) && fallbackModel) {
    const fb = await callChatAPI({
      ...
      maxTokens: Math.round(maxTokens * 0.8), timeoutMs: Math.round(timeoutMs * 0.8), extraHeaders
    });
...
function runGroq(messages, maxTokens, timeoutMs) {
...
function runOpenRouter(messages, maxTokens, timeoutMs) {
  return callWithRotation({
    ...
    messages, maxTokens, timeoutMs,
    extraHeaders: {
      'HTTP-Referer': process.env.OPENROUTER_SITE_URL || 'https://dream-lens.vercel.app',
      'X-Title': process.env.OPENROUTER_SITE_NAME || 'Dream Lens'
    }
  });
}
...
async function persistExchange({ userId, sessionId, userDream, aiText, category, entity, usageEntries }) {
  ...
  await sql`
    ...
    VALUES (${sid}, ${userId}, 'user', ${userDream}, ${category}, ${entity})
  `;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const knowledgeDirective = retrievedMeaning
      ? `\n[প্রেক্ষাপট তথ্য — এই স্বপ্নের প্রধান প্রতীক সম্পর্কে কিউরেটেড জ্ঞান: "${retrievedMeaning}" — "প্রতীক ও ব্যাখ্যা" অংশে এই তথ্যকে ভিত্তি করে লিখুন, এর বাইরে গিয়ে নিজে থেকে ভিন্ন প্রতীকী অর্থ বানাবেন না। ইউজারের নির্দিষ্ট প্রেক্ষাপটের (পেশা/অনুভূতি) সাথে মিলিয়ে এটাকে ব্যক্তিগতকরণ করুন।]`
      : '';
    ...
    if (inputType === 'context_answer') {
      ...
      contextDirective = '\n[সিস্টেম-নির্ধারিত ইনপুট-টাইপ: (গ) সাধারণ প্রশ্ন/কাজ (স্বপ্ন ছাড়া অন্য যেকোনো বিষয়)। নির্দিষ্ট কাঠামো ব্যবহার করবেন না, বিষয় অনুযায়ী সম্পূর্ণ ও সরাসরি উত্তর দিন।]';
    } else {
    ...
    if (inputType === 'general_question') {
      ...
      const r = await runGroq(messages, 3000, 28000);
      if (r.error || r.retry) {
      ...
      const [groqResult, orResult] = await Promise.all([
        runGroq(messages, 1400, 25000),
        runOpenRouter(messages, 1400, 25000)
      ]);
      ...
      if (groqOk && orOk) {
        ...
        const synth = await runGroq(synthesisMessages, 1400, 22000);
