...
async function persistExchange({ userId, sessionId, userDream, aiText, category, entity, usage, model }) {
  let sid = sessionId;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ================= যতগুলো GROQ_API_KEY_N env var সেট করা আছে, সবগুলো automatically লোড হয় =================
    // নতুন key যোগ করতে চাইলে শুধু Vercel env-এ GROQ_API_KEY_4, GROQ_API_KEY_5... যোগ করলেই হবে, কোড বদলাতে হবে না
    ...
    for (let i = 0; i < keys.length; i++) {
      const r = await callGroq(keys[i], 'llama-3.3-70b-versatile', messages, 1400, 28000);
      if (r.retry) continue;
    ...
    if (!result || result.error || result.retry) {
      const fallback = await callGroq(keys[0], 'llama-3.1-8b-instant', messages, 1100, 20000);
