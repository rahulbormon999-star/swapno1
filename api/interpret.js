...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (inputType === 'general_question') {
      ...
      const r = await runGroq(messages, 1600, 15000, 0.85, 2);
      if (r.error || r.retry) {
      ...
      const [groqResult, orResult] = await Promise.all([
        runGroq(messages, 1400, 10000, 0.85, 2),
        runOpenRouter(messages, 1400, 10000, 0.85, 2)
      ]);
      ...
      if (groqOk && orOk) {
        ...
        const synth = await runGroq(synthesisMessages, 1400, 10000, 0.75, 1);
