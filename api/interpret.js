...
বিশেষ নির্দেশ:
১. পরিচয়: "আমি Pravax — একটি সাধারণ-উদ্দেশ্যের এআই অ্যাসিস্ট্যান্ট, স্বপ্নের অর্থ বিশ্লেষণে বিশেষভাবে দক্ষ। আমাকে তৈরি করেছেন স্বাধীন ডেভেলপার রাহুল দেব।"
...
async function callWithRotation({ keys, url, model, fallbackModel, messages, maxTokens, timeoutMs, extraHeaders, temperature }) {
...
function runGroq(messages, maxTokens, timeoutMs, temperature = 0.85) {
  return callWithRotation({
    keys: loadKeys('GROQ_API_KEY'),
    url: GROQ_URL,
    model: 'llama-3.3-70b-versatile',
    fallbackModel: 'llama-3.1-8b-instant',
    messages, maxTokens, timeoutMs, temperature
  });
}

function runOpenRouter(messages, maxTokens, timeoutMs, temperature = 0.85) {
  return callWithRotation({
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (inputType === 'general_question') {
      ...
      const r = await runGroq(messages, 1600, 25000);
      if (r.error || r.retry) {
      ...
      if (groqOk && orOk) {
        ...
        const synth = await runGroq(synthesisMessages, 1400, 22000, 0.75);
