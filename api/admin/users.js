...
export default async function handler(req, res) {
    ...
    try {
      ...
      if (req.query.id) {
        const userRows = await sql`
          SELECT id, login_source, first_name, last_name, email, phone, profile_picture,
                 plan, banned, ban_reason, banned_at, created_at, last_login_at
          FROM users WHERE id = ${req.query.id}
        `;
        if (userRows.length === 0) return res.status(404).json({ error: 'ইউজার পাওয়া যায়নি' });
        ...
        const messages = await sql`
          SELECT id, session_id, role, content, category, feedback, created_at
          FROM dream_messages
      ...
      const rows = await sql`
        SELECT u.id, u.first_name, u.last_name, u.email, u.login_source, u.plan, u.profile_picture,
               u.banned, u.ban_reason, u.banned_at, u.created_at,
  ...
  // ================= POST: ban / unban / delete (একক বা bulk) =================
  if (req.method === 'POST') {
    try {
      const { id, ids, action, reason } = req.body || {};
      const targetIds = Array.isArray(ids) && ids.length > 0 ? ids : (id ? [id] : []);

      if (targetIds.length === 0 || !['delete', 'ban', 'unban'].includes(action)) {
